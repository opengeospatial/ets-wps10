...
import org.opengis.cite.wps10.ReusableEntityFilter;
...
public class ClientUtils {
	...
	public static Response buildGetRequest(URI endpoint, Map<String, String> qryParams, MediaType... mediaTypes) {
		return buildRequest(endpoint, qryParams, HttpMethod.GET, mediaTypes);
	}
	 ...
	 * @return A ClientRequest object.
	 */
	public static Response buildRequest(URI endpoint, Map<String, String> qryParams, String method,
		...
		Invocation req = reqBuilder.build(method);
		return req.invoke();
	}

	/**
	 * Builds an HTTP request message that uses the HEAD method. It is identical to GET
	 * except that the server does return a message body in the response.
	 * @param endpoint A URI indicating the target resource.
	 * @param qryParams A Map containing query parameters (may be null);
	 * @param mediaTypes A list of acceptable media types; if not specified, any
	 * representation ("*&#47;*") is acceptable.
	 * @return A ClientRequest object.
	 */
	public static Response buildHeadRequest(URI endpoint, Map<String, String> qryParams, MediaType... mediaTypes) {
		Response req = buildRequest(endpoint, qryParams, HttpMethod.HEAD, mediaTypes);
		return req;
	}
	...
	/**
	 * Determines if the given content type is compatible with any of the media types that
	 * are deemed to be acceptable. Parameters are ignored. If a general XML media type
	 * (application/xml or text/xml) is acceptable, then a more specific XML-based subtype
	 * (with the '+xml' suffix) will be accepted; however, the converse will be false
	 * (i.e. a more general type is not necessarily substitutable for a specific subtype).
	 * @param contentType A String denoting a content type.
	 * @param acceptableTypes A collection of acceptable MediaType specifiers.
	 * @return true if the content type is acceptable; false otherwise.
	 */
	public static boolean contentIsAcceptable(String contentType, MediaType... acceptableTypes) {
		...
		for (MediaType acceptType : acceptableTypes) {
			...
			if (acceptType.getSubtype().equals("xml") && type.getSubtype().endsWith("xml")) {
				isAcceptable = true;
				break;
			}
		}
