	...
	void processSuiteParameters(ISuite suite) {
		...
		if ((null == httpGetUriParam) || httpGetUriParam.isEmpty()) {
			throw new IllegalArgumentException(
					"Required test run parameter not found: " + TestRunArg.EXECUTE_HTTP_GET_URI.toString());
		}
		...
		try {
			...
			throw new RuntimeException("Failed to dereference resource located at " + executeRequestRawDataOutputRef,
					ex);
		}
		...
		// RESPONSE DOCUMENT
		String executeRequestResponseDocumentOutputParam = params
			.get(TestRunArg.EXECUTE_REQUEST_RESPONSE_DOCUMENT_URI.toString());
		...
		try {
			...
			throw new RuntimeException(
					"Failed to dereference resource located at " + executeRequestResponseDocumentOutputRef, ex);
		}
		Document executeRequestResponseDocumentOutputDoc = null;
		try {
			executeRequestResponseDocumentOutputDoc = URIUtils
				.parseURI(executeRequestResponseDocumentOutputFile.toURI());
		}
		catch (Exception ex) {
			throw new RuntimeException(
					"Failed to parse resource retrieved from " + executeRequestResponseDocumentOutputRef, ex);
		}
		suite.setAttribute(SuiteAttribute.EXECUTE_REQUEST_RESPONSE_DOCUMENT_URI.getName(),
				executeRequestResponseDocumentOutputDoc);

		// UPDATING RESPONSE DOCUMENT
		String executeRequestUpdatingResponseDocumentOutputParam = params
			.get(TestRunArg.EXECUTE_REQUEST_UPDATING_RESPONSE_DOCUMENT_URI.toString());
		URI executeRequestUpdatingResponseDocumentOutputRef = URI
			.create(executeRequestUpdatingResponseDocumentOutputParam.trim());
		File executeRequestUpdatingResponseDocumentOutputFile = null;
		try {
			executeRequestUpdatingResponseDocumentOutputFile = URIUtils
				.dereferenceURI(executeRequestUpdatingResponseDocumentOutputRef);
		}
		catch (Exception ex) {
			throw new RuntimeException(
					"Failed to dereference resource located at " + executeRequestUpdatingResponseDocumentOutputRef, ex);
		}
		Document executeRequestUpdatingResponseDocumentOutputDoc = null;
		try {
			executeRequestUpdatingResponseDocumentOutputDoc = URIUtils
				.parseURI(executeRequestUpdatingResponseDocumentOutputFile.toURI());
		}
		catch (Exception ex) {
			throw new RuntimeException(
					"Failed to parse resource retrieved from " + executeRequestUpdatingResponseDocumentOutputRef, ex);
		}
		suite.setAttribute(SuiteAttribute.EXECUTE_REQUEST_UPDATING_RESPONSE_DOCUMENT_URI.getName(),
				executeRequestUpdatingResponseDocumentOutputDoc);
	}
